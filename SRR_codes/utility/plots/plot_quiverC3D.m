function q_handle=plot_quiverC3D(x,y,z,u,v,w,varargin)
% This is a modified version of quiverC3D
...
%   would be obtained using q_handle=quiver3(x, y, z, u, v, w). I do not know
%   whether the scaling method is the same is in q_handle=quiver3.
%
%
% --------------------------------------------------------------------------------------
%
...
% --------------------------------------------------------------------------------------
%
%   See also: q_handle=quiver3, LINESPEC, COLORMAP.
%
%%
count_tick=5;
%% parse arguments
...
addOptional(p, 'Irange', [1 0], @isnumeric);
addOptional(p, 'Global', false, @islogical);
...
if(~glo)
    ...
    if(Ig(1)<Ig(2))
        ...
        error('Check the Ig values: Either Imin>=Imax or no value provided');
    end
end
...
if numel(u) > ncolors
    ...
    for k = 1:ncolors
        mask = (Ic == k);
        q_handle=quiver3(ax, x(mask), y(mask), z(mask), u(mask), v(mask), w(mask), 0, ...
    ...
    % linear indexing!
    for k = 1:numel(u)
        q_handle=quiver3(ax, x(k), y(k), z(k), u(k), v(k), w(k), 0, 'Color', C(Ic(k),:), ...
...
if p.Results.Colorbar
    ticks=linspace(Imin, Imax, count_tick);
    if(glo)
        cb = ax.Colorbar;
    ...
    set(cb,'Limits',[Imin,Imax],'ticks',ticks);
    tlabels = sprintfc('%1.3g ',ticks);
